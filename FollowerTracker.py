...
def obtainUser():
    ...
    global PROFILE
    USERNAME = input(
        f'Whose followers do you want to check? (If checking own followers, just hit ENTER): ')
    if USERNAME.strip() == '':
        CHECKING = 'you'
        USERNAME = USER
    else:
...
def readFile():
    ...
    file = open(
        f'/Users/{COMPUTER}/Documents/FollowerChecker/{USERNAME}\'sfollowers.txt', 'r')
    old_followers = set(file.read().splitlines())
    file.close()
    file = open(
        f'/Users/{COMPUTER}/Documents/FollowerChecker/{USERNAME}\'sfollowers.txt', 'w')
...
def main():
    ...
    if not os.path.exists(f'/Users/{COMPUTER}/Documents/FollowerChecker/{USERNAME}\'sfollowers.txt'):
        ...
        print(f'Followers list saved to {
              USERNAME}\'s_followers.txt in Documents\n')
    if CHECKING == 'you':
        reciprocators = input(
            f'Would you like to check who is not following you back? (y/n): ')
